...
export const createUser = async(req: Request, res: Response)=>{
  try {
    ...
    res.status(201).json({message:"User created successfully",id:newUser._id,name:newUser.name,email:newUser.email, token});
  } catch (error) {
...
export const LoginUser = async(req: Request, res: Response)=>{
  try {
    ...
    res.status(200).json({message:"User logged in successfully",id:existingUser._id,name:existingUser.name,email:existingUser.email, token});
  } catch (error) {
